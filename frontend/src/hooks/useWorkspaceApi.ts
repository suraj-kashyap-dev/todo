...
export const useWorkspaceApi = () => {
  ...
  const getWorkspaceById = async (id: string): Promise<Workspace | null> => {
    try {
      setState((prev) => ({ ...prev, loading: true, error: null }));
      const response = await httpClient.get<Workspace>(
        `${API_WORKSPACE}/${id}`,
      );
    ...
    try {
      ...
      setState((prev) => ({
        ...
        workspace: prev.workspace
          ? prev.workspace.filter((ws) => ws.id !== id)
          : null,
        currentWorkspace:
          prev.currentWorkspace?.id === id ? null : prev.currentWorkspace, // Clear current workspace if it was deleted
      }));
    } catch (error) {
      const errorMessage = handleError(error);
      setState((prev) => ({ ...prev, error: errorMessage }));
